namespace _06.Repainting
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            int neededDilutantLiters = int.Parse(Console.ReadLine());
            int completionHours = int.Parse(Console.ReadLine());


            double materialsPrice = ((neededNylon + 2) * 1.5) + ((neededPaintLiters * 1.10) * 14.50) + (neededDilutantLiters * 5.00) + 0.40;

            double workersMoney = completionHours * (materialsPrice * 0.30);

            double totalSum = materialsPrice + workersMoney;
